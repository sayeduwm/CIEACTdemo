...
#U_list = {rows[0]:rows[1] for rows in r3}
#B_list = {rows[0]:rows[1] for rows in r1}
def uniBigramClassifier(text_string,U_list,B_list,thrs,hybirdModel):
    text_string = text_string.lower()
    exclude = set(string.punctuation)
    text_string = ''.join(ch for ch in text_string if ch not in exclude)
    unigrams = text_string.split()    
...
def ClassifierWthrs(text_string,U_list,B_list,T_list,hybirdModel,Uthrs,Bthrs,Tthrs):
    NoisyORU,NoisyORB,NoisyORT,NoisyORUB=None,None,None,None
    Positive_Unigams,Positive_Bigams,Positive_Trigams=[],[],[]
    text_for_viz=text_string
    textViz_output=''
    text_string = text_string.lower()
    exclude = set(string.punctuation)
    text_string = ''.join(ch for ch in text_string if ch not in exclude)
    ...
    if hybirdModel=="UB": 
        ...
        textViz_output=textViz(text_for_viz,U_list,B_list)
        
    return NoisyORU,NoisyORB,NoisyORT,NoisyORUB,Positive_Unigams,Positive_Bigams,Positive_Trigams,textViz_output
